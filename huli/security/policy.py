...
class SecurityPolicy:
    ...
    def validate_password(self, password: str) -> None:
        ...
        if len(password) < self.min_password_length:
            raise ValueError(f"A senha é opcional. Se você escolher usar uma, ela precisa ter pelo menos {self.min_password_length} caracteres.")

    def validate_input(self, text: str) -> None:
        if len(text) > self.max_input_chars:
            raise ValueError(f"A mensagem excede o limite de {self.max_input_chars} caracteres.")
