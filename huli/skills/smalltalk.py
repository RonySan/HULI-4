...
class SmallTalkSkill:
    ...
    def handle(self, request: KernelRequest) -> KernelResponse:
        ...
        suffix = f", {username}" if username and username.casefold() != "visitante" else ""
        if "quem e voce" in text or "quem e a huli" in text:
            answer = "Sou a Huli, sua assistente pessoal. Nesta fase já consigo manter contexto curto, tarefas, agenda e algumas conversas locais."
        ...
        elif any(word in text for word in ("obrigado", "obrigada", "valeu", "agradecido", "agradecida")):
        ...
        return KernelResponse(request_id=request.request_id, text=answer, handled_by=self.name)
