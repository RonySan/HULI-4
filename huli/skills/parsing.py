...
def extract_cancel_target(text: str) -> str:
    value = strip_huli_prefix(text)
    value = re.sub(
        r"^(?:cancela|cancelar|cancele|remove|remover|remova)\s+",
        "",
        value,
        flags=re.IGNORECASE,
    )
    value = re.sub(
        r"^(?:o\s+)?(?:compromisso|evento|agendamento)\s*",
        "",
        value,
        flags=re.IGNORECASE,
    )
...
def parse_appointment_request(
    ...
    *,
    ...
    time_match = re.search(
        r"(?:\bas\b|\bàs\b)\s*(\d{1,2})(?::(\d{2}))?\s*(?:h|horas)?\b",
        raw,
        re.IGNORECASE,
    )
    ...
    if "amanha" in normalized:
        ...
        if date_match:
            ...
            if year < 100:
                year += 2000
            try:
                target_date = reference.replace(
                    year=year,
                    month=month,
                    day=day,
                ).date()
            except ValueError as exc:
                raise ValueError("A data do compromisso é inválida.") from exc
        elif day_match:
            ...
            for _ in range(13):
                ...
                if candidate is not None and candidate >= reference.date():
