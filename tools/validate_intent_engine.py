...
def run_validation() -> None:
    ...
    cases = (
        ...
        ("agenda dentista amanhã às 15:00", IntentName.AGENDA_CREATE),
        ("tarefas pendentes", IntentName.TASK_LIST),
        ("oi huli, bom dia", IntentName.SMALL_TALK),
        ("vamos falar do projeto Medynx", IntentName.PROJECT_SET),
    ...
    with TemporaryDirectory(prefix="huli-intent-") as temp_dir:
        ...
        meta = {
            "session_id": "intent-validation",
            "username": "rony",
            "role": "owner",
        }
        require(
            runtime.kernel.process("que horas são?", metadata=meta).handled_by == "time",
            "TimeSkill não recebeu time.query.",
        )
        require(
            runtime.kernel.process("oi huli, bom dia", metadata=meta).handled_by
            == "smalltalk",
            "SmallTalkSkill não recebeu smalltalk.",
        )
