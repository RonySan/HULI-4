...
def run_validation() -> None:
    with TemporaryDirectory(prefix="huli-foundation-") as temp_dir:
        runtime = build_runtime(Settings(environment="validation", log_level="CRITICAL", data_dir=Path(temp_dir) / "data", session_hours=1))
        ...
        require(user.id == owner.id and bool(token), "Autenticação da fundação falhou.")
        response = runtime.kernel.process("ping", metadata={"session_id": "phase0-regression"})
        require(response.handled_by == "foundation", "Ping não chegou à FoundationSkill.")
        client = TestClient(create_app(runtime))
        require(client.get("/health").json()["version"] == __version__, "Health retornou versão incorreta.")
...
def main() -> int:
    try:
        ...
        print(f"FUNDAÇÃO: FALHA - {exc}"); return 1
