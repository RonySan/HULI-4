...
from huli.infrastructure import EventRepository, InteractionRepository, RuntimeRecorder, SQLiteDatabase
...
def test_runtime_recorder_persists_interaction(tmp_path: Path) -> None:
    database = SQLiteDatabase(tmp_path / "huli.db")
    database.initialize()
    events = EventBus(); event_repository = EventRepository(database); interactions = InteractionRepository(database)
    RuntimeRecorder(events, event_repository, interactions)
    events.publish("kernel.request.received", {"request_id": "abc", "text": "ping"})
    events.publish("kernel.response.created", {"request_id": "abc", "text": "pong", "handled_by": "foundation", "ok": True})
